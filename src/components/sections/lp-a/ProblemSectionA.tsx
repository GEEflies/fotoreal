    ...
    <section ref={ref as React.RefObject<HTMLElement>} className="section-padding">
      <div className={`section-container transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className="grid md:grid-cols-3 gap-5 sm:gap-8">
          {painPoints.map((p, i) => (
            <div key={i} className="relative p-5 sm:p-6 rounded-xl bg-destructive/[0.04] border border-destructive/10 hover:border-destructive/20 transition-colors">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="font-heading font-semibold text-foreground text-base sm:text-lg">{p.title}</h3>
              </div>
